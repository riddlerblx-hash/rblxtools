...
function createCodesPlatform({ dataPath, provider = new DisabledRblxdbProvider() }) {
  ...
  return {
    provider,
    list({ search = "", limit = 20, page = 1 } = {}) { const data = read(); const query = clean(search, 100).toLowerCase(); const games = data.games.filter((game) => game.codesEnabled && (!query || `${game.name} ${game.slug}`.toLowerCase().includes(query))).sort((a, b) => String(b.createdAt).localeCompare(String(a.createdAt))); const size = Math.max(1, Math.min(Number(limit) || 20, 100)); const current = Math.max(1, Number(page) || 1); return { games: games.slice((current - 1) * size, current * size).map((game) => publicGame(game, data)), page: current, limit: size, total: games.length, provider: { id: provider.id, enabled: provider.enabled, reason: provider.reason || null } }; },
    listRanked({ period = "popular", limit = 8 } = {}) { return { games: [], period, limit: Math.max(1, Math.min(Number(limit) || 8, 24)), provider: { id: provider.id, enabled: provider.enabled, reason: provider.reason || null } }; },
    getGame,
    upsertGame(input) { const data = read(); const slug = slugify(input.slug || input.name); if (!slug) throw new Error("A game name or slug is required."); let game = data.games.find((item) => item.slug === slug || (input.robloxUniverseId && item.robloxUniverseId === String(input.robloxUniverseId))); const now = new Date().toISOString(); if (!game) { game = { id: randomUUID(), createdAt: now }; data.games.push(game); } Object.assign(game, { name: clean(input.name, 140), slug, robloxUniverseId: clean(input.robloxUniverseId, 60) || null, robloxPlaceId: clean(input.robloxPlaceId, 60) || null, robloxUrl: clean(input.robloxUrl, 500) || null, iconUrl: clean(input.iconUrl, 500) || null, description: clean(input.description, 600), redemptionInstructions: clean(input.redemptionInstructions, 4000), instructionImageUrls: Array.isArray(input.instructionImageUrls) ? input.instructionImageUrls.map((url) => clean(url, 500)).filter(Boolean).slice(0, 6) : [], codesEnabled: input.codesEnabled !== false, updatedAt: now }); write(data); return game; },
    ...
    updateCodeStatus(gameId, codeId, status) { const data = read(); const game = data.games.find((item) => item.id === gameId); const code = data.codes.find((item) => item.id === codeId && item.gameId === gameId); if (!game || !code) return null; const now = new Date().toISOString(); code.status = VALID_STATUS.has(status) ? status : code.status; code.expiredAt = code.status === "expired" ? now : null; code.updatedAt = now; game.updatedAt = now; write(data); return code; },
    deleteCode(gameId, codeId) { const data = read(); const index = data.codes.findIndex((item) => item.id === codeId && item.gameId === gameId); if (index < 0) return null; const [code] = data.codes.splice(index, 1); const game = data.games.find((item) => item.id === gameId); if (game) game.updatedAt = new Date().toISOString(); write(data); return code; },
