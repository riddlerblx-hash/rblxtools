...
function buildDashboard(store, appUserId) {
  ...
  return {
    ...
    server: server ? { guildId: server.guildId, guildName: server.guildName || "Discord server", claimedAt: server.claimedAt || null, perUserDailyLimit: Math.max(0, Number(server.perUserDailyLimit || 0)), dailyLimitEnabled: Boolean(server.dailyLimitEnabled), perUserWeeklyLimit: Math.max(0, Number(server.perUserWeeklyLimit || 0)), weeklyLimitEnabled: Boolean(server.weeklyLimitEnabled), perUserMonthlyLimit: Math.max(0, Number(server.perUserMonthlyLimit || 0)), monthlyLimitEnabled: Boolean(server.monthlyLimitEnabled), roleDailyLimits: normalizeRoleDailyLimits(server.roleDailyLimits), roleDailyLimitsEnabled: Boolean(server.roleDailyLimitsEnabled), paused: Boolean(server.paused), blockedCommands: normalizeBlockedCommands(server.blockedCommands), alertsEnabled: Boolean(server.alertsEnabled), alertThresholds: normalizeAlertThresholds(server.alertThresholds), alertChannelId: String(server.alertChannelId || ""), alertChannels: normalizeAlertChannels(server.alertChannels), activityByDate: trimActivity(server), auditLog: Array.isArray(server.auditLog) ? server.auditLog.slice(0, 30) : [], dailyUserUseCounts: server.dailyUserUseCounts || {} } : null,
  };
}
...
async function updateServerSettings({ appUserId, guildId, perUserDailyLimit, dailyLimitEnabled, perUserWeeklyLimit, weeklyLimitEnabled, perUserMonthlyLimit, monthlyLimitEnabled, roleDailyLimits, roleDailyLimitsEnabled }) {
  const userId = String(appUserId || "").trim(); const normalizedGuildId = String(guildId || "").trim(); const limit = Number.parseInt(perUserDailyLimit, 10); const weeklyLimit = Number.parseInt(perUserWeeklyLimit, 10); const monthlyLimit = Number.parseInt(perUserMonthlyLimit, 10);
  if (!/^\d+$/.test(normalizedGuildId) || ![limit, weeklyLimit, monthlyLimit].every((value) => Number.isFinite(value) && value >= 0 && value <= 1000)) { const error = new Error("Choose daily, weekly, and monthly user limits from 0 to 1,000."); error.statusCode = 400; throw error; }
  ...
  return updateStore((store) => { const server = store.serversByGuildId[normalizedGuildId]; if (!server || server.appUserId !== userId || server.unclaimedAt) { const error = new Error("You do not manage that Discord server."); error.statusCode = 403; throw error; } server.perUserDailyLimit = limit; server.dailyLimitEnabled = Boolean(dailyLimitEnabled); server.perUserWeeklyLimit = weeklyLimit; server.weeklyLimitEnabled = Boolean(weeklyLimitEnabled); server.perUserMonthlyLimit = monthlyLimit; server.monthlyLimitEnabled = Boolean(monthlyLimitEnabled); server.roleDailyLimits = normalizeRoleDailyLimits(roleDailyLimits); server.roleDailyLimitsEnabled = Boolean(roleDailyLimitsEnabled); addAudit(server, "limits_updated", "Usage limits updated"); server.updatedAt = new Date().toISOString(); return buildDashboard(store, userId); });
}
...
async function consumeDiscordServerUse({ guildId, discordUserId, discordRoleIds, commandName }) {
  ...
  return updateStore((store) => {
    ...
    const today = dailyCounts[dateKey] && typeof dailyCounts[dateKey] === "object" ? dailyCounts[dateKey] : {}; const alreadyUsed = Math.max(0, Number(today[memberId] || 0)); const matchingRoleLimit = server.roleDailyLimitsEnabled ? normalizeRoleDailyLimits(server.roleDailyLimits).filter((entry) => memberRoleIds.has(entry.roleId)).reduce((highest, entry) => Math.max(highest, entry.dailyLimit), 0) : 0; const baseLimit = server.dailyLimitEnabled ? Math.max(0, Number(server.perUserDailyLimit || 0)) : 0; const limit = Math.max(baseLimit, matchingRoleLimit); const weeklyUses = periodUseCount(activity, dateKey, memberId, "week"); const monthlyUses = periodUseCount(activity, dateKey, memberId, "month"); const weeklyLimit = server.weeklyLimitEnabled ? Math.max(0, Number(server.perUserWeeklyLimit || 0)) : 0; const monthlyLimit = server.monthlyLimitEnabled ? Math.max(0, Number(server.perUserMonthlyLimit || 0)) : 0;
    ...
    if (weeklyLimit > 0 && weeklyUses >= weeklyLimit) { const error = new Error("You reached this server's weekly RBLXTools command limit."); error.statusCode = 429; throw error; }
    if (monthlyLimit > 0 && monthlyUses >= monthlyLimit) { const error = new Error("You reached this server's monthly RBLXTools command limit."); error.statusCode = 429; throw error; }
