...
async function getDiscordServerUsageSummary({ guildId, discordUserId, discordRoleIds }) {
  ...
  return { mode: dashboard.mode, totalUses: dashboard.totalUses, usedUses: dashboard.usedUses, remainingUses: dashboard.remainingUses, userLimit: userLimit > 0 ? { period: userPeriod, used: useCountFor(userPeriod), limit: userLimit } : null, roleLimit: roleLimit > 0 ? { period: rolePeriod, used: useCountFor(rolePeriod), limit: roleLimit } : null };
}
...
async function consumeDiscordServerUse({ guildId, discordUserId, discordRoleIds, commandName }) {
  ...
  return updateStore((store) => {
    ...
    if (userLimit > 0 && useCountFor(userPeriod) >= userLimit) { const error = new Error("You reached this server's " + userPeriod + " RBLXTools command limit."); error.statusCode = 429; throw error; }
    if (roleLimit > 0 && useCountFor(rolePeriod) >= roleLimit) { const error = new Error("You reached this server's role " + rolePeriod + " RBLXTools command limit."); error.statusCode = 429; throw error; }
