...
function buildDashboard(store, appUserId) {
  ...
  return {
    ...
    server: server ? { guildId: server.guildId, guildName: server.guildName || "Discord server", claimedAt: server.claimedAt || null, perUserDailyLimit: Math.max(0, Number(server.perUserDailyLimit || 0)), dailyLimitEnabled: Boolean(server.dailyLimitEnabled), roleDailyLimits: normalizeRoleDailyLimits(server.roleDailyLimits), roleDailyLimitsEnabled: Boolean(server.roleDailyLimitsEnabled), paused: Boolean(server.paused), blockedCommands: normalizeBlockedCommands(server.blockedCommands), alertsEnabled: Boolean(server.alertsEnabled), alertThresholds: normalizeAlertThresholds(server.alertThresholds), alertChannelId: String(server.alertChannelId || ""), alertChannels: normalizeAlertChannels(server.alertChannels), activityByDate: trimActivity(server), auditLog: Array.isArray(server.auditLog) ? server.auditLog.slice(0, 30) : [], dailyUserUseCounts: server.dailyUserUseCounts || {} } : null,
  };
}
...
async function updateServerSettings({ appUserId, guildId, perUserDailyLimit, dailyLimitEnabled, roleDailyLimits, roleDailyLimitsEnabled }) {
