(function () {
    ...
    return (
      '<div class="rblx-shell-membership-gate" id="rblxShellAnimationGate" aria-hidden="true">' +
        '<div class="rblx-shell-membership-gate-card" role="dialog" aria-modal="true" aria-labelledby="rblxShellAnimationGateTitle">' +
          ...
          '<div class="rblx-shell-gate-benefits"><section><strong>Plus</strong><span>30 AI Credits Every Month</span><span>Animation Studio</span><span>Bulk Downloads (1-5)</span><span>Much More</span></section><section><strong>Pro</strong><span>Everything from Plus</span><span>200 AI Credits Every Month</span><span>No Annoying Ads</span><span>RBLXTools Discord Bot Access</span><span>Bulk Downloads (5-10)</span><span>Much More to Offer</span></section></div>' +
  ...
  function previewProfile(profile, eventOrAnchor) {
    ...
    if (profileId && !cachedStats && !profilePreviewStatsCache[profileId + ':loading']) {
      ...
      fetch('/api/members/' + encodeURIComponent(profileId), { credentials: 'include', headers: getToken() ? { Authorization: 'Bearer ' + getToken() } : {} }).then(function (response) { return response.ok ? response.json() : null; }).then(function (payload) {
        if (!payload) return;
        profilePreviewStatsCache[profileId] = { followerCount: payload.followerCount, followingCount: payload.followingCount, canViewMemberId: Boolean(payload.canViewMemberId) };
        if (requestId === profilePreviewRequest && profilePreview && !profilePreview.hidden && profilePreview.dataset.memberId === profileId) {
          ...
          var memberIdLine = profilePreview.querySelector('.rblx-shell-profile-preview-id');
          if (memberIdLine && payload.canViewMemberId) { memberIdLine.hidden = false; memberIdLine.textContent = 'User ID: ' + profileId; }
        }
      }).catch(function () {}).finally(function () { delete profilePreviewStatsCache[profileId + ':loading']; });
    }
  ...
  // A login should always return to this site. Do not trust a return path
  // supplied by any page or third-party script.
  function getSafeAuthReturnUrl(value) {
    var fallback = getCleanCurrentUrl();
    try {
      var destination = new URL(String(value || fallback), window.location.href);
      return destination.origin === window.location.origin ? destination.href : fallback;
    } catch (_error) {
      return fallback;
    }
  }
  ...
  function openAuthModal(options) {
    ...
    shellState.authReturnUrl = getSafeAuthReturnUrl(options.returnTo);
    setAuthMode(mode);
    if (shellState.authPassword) {
  ...
  function finishAuthSuccess(result, successMessage) {
    ...
    window.setTimeout(function () {
      var destination = getSafeAuthReturnUrl(shellState.authReturnUrl);
      closeAuthModal();
    ...
    if (state.loggedIn) {
      fetch(API_BASE + "/referrals/me", { credentials: "include", headers: { Authorization: "Bearer " + getToken() } })
        ...
        .then(function (payload) {
          ...
          if (balance && payload && payload.referral) balance.textContent = "$" + ((Number(payload.referral.totalBalanceCents != null ? payload.referral.totalBalanceCents : payload.referral.availableCents) || 0) / 100).toFixed(2);
        }).catch(function () {});
    }
    refreshCommunityNotifications();
    if (shellState.chatAdminButton) {
  ...
  function enhanceRewardsGiftCardPicker() {
    ...
    setTimeout(function () {
      ...
      redeem.onclick = async function () {
        if (cashMode) {
          if (pointBalance < 1000) return alert("You need at least 1,000 approved RBLX Points to convert RBLXTools Cash.");
          redeem.disabled = true;
          try {
            var cashResponse = await fetch("/api/rewards/redeem-cash", { method: "POST", credentials: "include", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ cashCents: selectedCashCents }) });
            var cashPayload = await cashResponse.json().catch(function () { return {}; });
            if (!cashResponse.ok) throw Error(cashPayload.error || "Could not convert your points.");
            pointBalance = Math.max(0, Number(cashPayload.rewardPoints) || 0);
            var pointsElement = document.getElementById("points"); if (pointsElement) pointsElement.textContent = pointBalance.toLocaleString();
            try { window.dispatchEvent(new CustomEvent("rblxtools-reward-points-balance", { detail: { rewardPoints: pointBalance } })); } catch (_) {}
            fetch("/referrals/me", { credentials: "include" }).then(function (response) { return response.ok ? response.json() : null; }).then(function (payload) { var balance = document.getElementById("rblxShellReferralBalance"); if (balance && payload && payload.referral) balance.textContent = "$" + ((Number(payload.referral.totalBalanceCents) || 0) / 100).toFixed(2); }).catch(function () {});
